...
void func_8006FC88(u8 arg0, EnvironmentContext* envCtx, SkyboxContext* skyboxCtx) {
    ...
    if (arg0 == 5) {
        ...
        for (i = 0; i < ARRAY_COUNT(D_8011FC1C[envCtx->gloomySky]); i++) {
            ...
            if (gSaveContext.environmentTime >= entry->startTime &&
                (gSaveContext.environmentTime < entry->endTime || entry->endTime == 0xFFFF)) {
                if (entry->unk_04 != 0) {
                    envCtx->unk_13 =
                        Kankyo_InvLerp_u16(entry->endTime, entry->startTime, gSaveContext.environmentTime) * 255;
                } else {
                    envCtx->unk_13 = 0;
                }
                break;
...
void func_80070718(GlobalContext* globalCtx, Gfx** gfx) {
    ...
    // clang-format off
    numDays = gSaveContext.numDays; GfxPrint_Printf(&printer, "T%03d ", numDays);
...
void func_80070C24(GlobalContext* globalCtx, EnvironmentContext* envCtx, LightContext* lightCtx, PauseContext* pauseCtx,
    ...
    f32 f14 = 0.0f;
    LightSettings* lightSettingsList = globalCtx->envCtx.lightSettingsList;
    ...
    if (pauseCtx->state == 0) {
        ...
        if (SREG(0) != 0 || CREG(2) != 0) {
            Gfx* gfx;                                          // 74(sp)
            Gfx* polyOpa;                                      // 70(sp)
            GraphicsContext* gfxCtx = globalCtx->state.gfxCtx; // 6c(sp)
            Gfx* dispRefs[4];                                  // 5c(sp)
        ...
        if (envCtx->unk_BF != 0xFE) {
            if (envCtx->unk_1E == 0 && envCtx->unk_BF == 0xFF) { // 2048
                u16 i;
                for (i = 0; i < ARRAY_COUNT(*D_8011FB48); i++) { // 207c
                    if (gSaveContext.environmentTime >= D_8011FB48[envCtx->unk_1F][i].startTime &&
                        (gSaveContext.environmentTime < D_8011FB48[envCtx->unk_1F][i].endTime ||
                         D_8011FB48[envCtx->unk_1F][i].endTime == 0xFFFF)) {
                        f32 t =
                            Kankyo_InvLerp_u16(D_8011FB48[envCtx->unk_1F][i].endTime,
                                               D_8011FB48[envCtx->unk_1F][i].startTime, gSaveContext.environmentTime);
                        u8 temp;
                        ...
                        if (envCtx->unk_21 != 0) {
                            f14 = envCtx->unk_24;
                            f14 = (f14 - envCtx->unk_22) / f14;
                            if (--envCtx->unk_22 <= 0) {
                                envCtx->unk_21 = 0;
                                envCtx->unk_1F = envCtx->unk_20;
                            }
                        }
                        temp = lightSettingsList[D_8011FB48[envCtx->unk_1F][i].unk_04].ambientClrR;
                        consume((f32)temp, (f32)(lightSettingsList[D_8011FB48[envCtx->unk_1F][i].unk_05].ambientClrR - temp), f14);
                    }
                }
                // 31f0
            } else {
